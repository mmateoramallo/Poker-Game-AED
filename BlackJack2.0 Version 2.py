    ...
    while opcion != 3:
        ...
        if opcion == 1:
            ...
            if band_black_nat and (cant_cartas_croupier >= 3 and puntuacion_croupier == 21):
                ...
                # EL croupier esta mas cerca del 21 pero siendo su puntuacion menor o igual a 21
                ...
                # El jugador esta mas cerca del 21 pero siendo su puntuacion menor o igual a 21
                elif (distancia_player < distancia_croupier) and puntuacion_jugador <= 21:
                ...
                # Veo si hay empate
                    ...
                    monto += apuesta_juego
                    print(print('-' * 21, 'Su Monto Actual es de:', monto, '$', '-' * 21))
                    racha_croupier = False
                    ...
                    """
            # Evaluo si existio BlackJack Natural:
            if band_black_nat:
                if cant_cartas_croupier >= 3:
                    print(print('♠' * 21, 'Felicidades', player, ' Ganaste!!!!', '♠' * 21))
                    ganancias = apuesta_juego * 2
                    # Añado las ganancias al monto/pozo
                    monto += ganancias
                    print(print('-' * 21, 'Su Monto Actual es de:', monto, '-' * 21))
                    cant_victorias_jugador += 1
            elif band_black_nat_croup:
                if cant_cartas_jugador >= 3:
                    print(print('-' * 21, 'Intenta Nuevamente el ganador es el Croupier!!!!', '-' * 21))
                    ganancias_croupier = apuesta_juego
                    # Muestro el monto
                    print(print('-' * 21, 'Su Monto Actual es de:', monto, '-' * 21))
                    cant_victorias_croupier += 1
                ...
                if (distancia_player < distancia_croupier) and puntuacion_jugador < 21:
                    ...
                    ganancias_croupier = apuesta_juego
                    print(print('-' * 21, 'Su Monto Actual es de:', monto, '-' * 21))
                    print('-' * 21, 'El croupier gana: ', ganancias_croupier, '$')
                    print()
                    print('-' * 21, 'El ganador es', ganador, '-' * 21)
                    cant_victorias_croupier += 1

                elif (puntuacion_jugador <= 21 and (puntuacion_croupier > 21 or puntuacion_croupier < 21)):
                    print('♠' * 21, 'Felicidades!!!!', '♠' * 21)
                    ganancias = apuesta_juego * 2
                    # Añado las ganancias al monto/pozo
                    monto += ganancias
                ...
                elif (puntuacion_croupier <= 21 and (puntuacion_jugador > 21 or puntuacion_jugador < 21)):
                    print('-' * 21, 'Intenta Nuevamente!!!!', '-' * 21)
                    ganador = 'Croupier'
                    ganancias_croupier = apuesta_juego
                    print(print('-' * 21, 'Su Monto Actual es de:', monto, '-' * 21))
                ...
                elif (puntuacion_croupier > 21 and puntuacion_jugador > 21):
                    print('-' * 21, 'No gano ninguno, intenten nuevamente', '-' * 21)
                    monto += apuesta_juego
                    print(print('-' * 21, 'Su Monto Actual es de:', monto, '-' * 21))
                elif puntuacion_croupier == puntuacion_jugador:
                    print('-' * 21, 'EMPATE!!', '-' * 21)
                    monto += apuesta_juego
                    print(print('-' * 21, 'Su Monto Actual es de:', monto, '-' * 21))
                """
